...
function MakeCMSSW {
	...
	if [ ! -d $CondorSrc ]; then mkdir $myCondorWorkDir/input ; mkdir $myCondorWorkDir/input/src; fi
...
function MakeJob {
   ...
   mkdir ${myCondorWorkDir}/condorLog
...
EOF
	...
	if [ "$AutoSubmit" == "1" ]; then
		...
		if [ "$host" == "ui02" ] || [ "$host" == "ui02.sdfarm.kr" ]; then
			...
			name=${bTarFile/./}
			name=${name/.tgz/}
			ssh -p 4280 $host "if [ ! -d $hostDir ]; then mkdir $hostDir ; fi"
...
do
	...
	if [ "$ok" == "1" ]; then
		...
		((nNot++));
		if [ ! -d ${dir}/fail/condorLog ]; then mkdir ${dir}/fail/condorLog ; fi
		if [ ! -d ${dir}/fail/condorOut ]; then mkdir ${dir}/fail/condorOut ; fi
		if [ ! -d ${dir}/fail/cmsRunLog ]; then mkdir ${dir}/fail/cmsRunLog ; fi
